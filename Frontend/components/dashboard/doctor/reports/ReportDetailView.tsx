...
const fetchReportDetailClient = async (id: string): Promise<ReportDetailData> => {
...
interface ReportDetailViewProps {
    caseId: string;
    initialData: ReportDetailData | null;
}

export function ReportDetailView({ caseId, initialData }: ReportDetailViewProps) {
    const router = useRouter();

    const { data: report, isLoading, error } = useQuery({
        queryKey: ['report', caseId],
        queryFn: () => fetchReportDetailClient(caseId),
