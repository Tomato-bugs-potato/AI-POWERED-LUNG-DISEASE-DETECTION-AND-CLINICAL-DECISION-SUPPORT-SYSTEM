...
export function PatientDetailView({ patientId, initialData }: PatientDetailViewProps) {
    const router = useRouter();

    const { data: patient = initialData, isLoading } = useQuery({
    ...
    const activeCases = patient.history.filter((c: Case) => c.status !== 'Completed' && c.status !== 'Diagnosed');
    const pastCases = patient.history.filter((c: Case) => c.status === 'Completed' || c.status === 'Diagnosed');
